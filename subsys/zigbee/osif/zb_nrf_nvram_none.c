...
/* ZBOSS collaout that should be called once flash erase page operation is finished. */
...
zb_ret_t zb_osif_addr_read(zb_uint32_t address, zb_uint16_t len, zb_uint8_t *buf)
{
	...
	if ((p_address >= &zb_nvram_buf[0][0]) &&
	    ((p_address + len) <= &zb_nvram_buf[CONFIG_ZIGBEE_NVRAM_PAGE_COUNT - 1]
			    ...
			    zb_uint16_t len)
{
	if ((page < zb_get_nvram_page_count()) &&
	    ((pos + len) < zb_get_nvram_page_length())) {
